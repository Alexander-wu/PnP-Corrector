...
def setup(params):
    ...
    if params.normalization == 'zscore': 
        ...
        params.means_land = np.load(params.global_means_path_land)
        params.stds_land = np.load(params.global_stds_path_land)
    ...
    if params.nettype == 'DSLCast':
        ...
        from networks.DSLCast_land import DSLCast as model4
    ...
    checkpoint_file4  = params['best_checkpoint_path4']
    ...
    logging.info('Loading trained model4 checkpoint from {}'.format(checkpoint_file4))
    ...
    model4 = model4(params).to(device) 
    model4 = load_model(model4, params, checkpoint_file4)
    ...
    files_paths_land = glob.glob(params.test_data_path_land + "/*.h5")
    files_paths_land.sort()
    ...
    logging.info('Inference data_land from {}'.format(files_paths_land[yr]))
    climate_mean = np.load('/apdcephfs_qy3/share_301734960/easyluwu/gy/data/pnp/coupled_1_23layers/climate_mean_s_t_ssh.npy', mmap_mode='r')
    ...
    valid_data_full_land = h5py.File(files_paths_land[yr], 'r')['fields'][:365, :, :, :]

    return valid_data_full_atmos, valid_data_full_ocean, valid_data_full_sst, valid_data_full_land, model, model2, model4, model3, climate_mean

    
def autoregressive_inference(params, init_condition, valid_data_full_atmos, valid_data_full_ocean, valid_data_full_sst, valid_data_full_land, model, model2, model4, model3, climate_mean): 
    device = torch.cuda.current_device() if torch.cuda.is_available() else 'cpu'
    ...
    atmos_channels = np.array(params.atmos_channels)
    ocean_channels = np.array(params.ocean_channels)
    land_channels = np.array(params.land_channels)
    n_atmos_channels  = len(atmos_channels)
    n_ocean_channels  = len(ocean_channels)
    n_land_channels  = len(land_channels)
    
    seq_real        = torch.zeros((prediction_length, n_atmos_channels + n_ocean_channels + n_land_channels, img_shape_x, img_shape_y))
    seq_pred        = torch.zeros((prediction_length, n_atmos_channels + n_ocean_channels + n_land_channels, img_shape_x, img_shape_y))
    ...
    valid_data_land = valid_data_full_land[icd:(icd+prediction_length*dt+n_history*dt):dt][:, params.land_channels][:,:,0:180]
    ...
    logging.info(f'valid_data_full_land: {valid_data_full_land.shape}')
    logging.info(f'valid_data_land: {valid_data_land.shape}')
    ...
    if params.normalization == 'zscore': 
        ...
        valid_data_land = (valid_data_land - params.means_land[:,params.land_channels])/params.stds_land[:,params.land_channels]
        valid_data_land = np.nan_to_num(valid_data_land, nan=0)
        
        
    valid_data_atmos = torch.as_tensor(valid_data_atmos)
    valid_data_ocean = torch.as_tensor(valid_data_ocean)
    valid_data_land = torch.as_tensor(valid_data_land)
    ...
    with torch.no_grad():
        for i in range(valid_data_atmos.shape[0]): 
            if i==0: # start of sequence, t0 --> t0'
                ...
                first_land = valid_data_land[0:n_history+1]
                first_sst = valid_data_sst[0:n_history+1]
                first = torch.cat((first_atmos, first_ocean, first_land), axis=1)
                ic(first_atmos.shape, first_ocean.shape, first_land.shape, first.shape)
                future_atmos = valid_data_atmos[n_history+1]
                future_ocean = valid_data_ocean[n_history+1]
                future_land = valid_data_land[n_history+1]
                future = torch.cat((future_atmos, future_ocean, future_land), axis=0)
                ic(future.shape)
                ...
                first_land = first_land.to(device, dtype=torch.float)
                ...
                atmos_forcing_for_land = first_atmos[:, [65, 66, 67], :, :]
                model4_input = torch.cat((first_land, atmos_forcing_for_land), axis=1)
                model4_future_pred = model4(model4_input)

                # future_pred = torch.cat((model_future_pred, model2_future_pred, model4_future_pred), axis=1)

                model3_input = torch.cat((model_future_pred, model2_future_pred, model4_future_pred), axis=1)
                model3_future_pred = model3(model3_input)
                model3_future_pred[:, 69:162, :, :] = torch.masked_fill(input=model3_future_pred[:, 69:162, :, :], mask=~mask_data, value=0)
                ...
                if i < prediction_length-1:
                    ...
                    future_land = valid_data_land[n_history+i+1]
                    future = torch.cat((future_atmos, future_ocean, future_land), axis=0)
                    ic(future.shape)
                ...
                atmos_forcing1 = model_future_pred[:, [65, 66, 67, 68], :, :]
                ocean_input = future_pred[:, 69:162, :, :]
                ...
                atmos_forcing_for_land = atmos_input[:, [65, 66, 67], :, :]
                land_input = future_pred[:, 162:, :, :]
                model4_input = torch.cat((land_input.to(device, dtype=torch.float), atmos_forcing_for_land.to(device, dtype=torch.float)), axis=1).to(device, dtype=torch.float)
                model4_future_pred = model4(model4_input)

                # future_pred = torch.cat((model_future_pred, model2_future_pred, model4_future_pred), axis=1)

                model3_input = torch.cat((model_future_pred, model2_future_pred, model4_future_pred), axis=1).to(device, dtype=torch.float)
                model3_future_pred = model3(model3_input)
                model3_future_pred[:, 69:162, :, :] = torch.masked_fill(input=model3_future_pred[:, 69:162, :, :], mask=~mask_data, value=0)
    ...
    seq_real[:, 69:162, :, :] = seq_real[:, 69:162, :, :] * params.stds_ocean[:,params.ocean_channels] + params.means_ocean[:,params.ocean_channels]
    seq_real[:, 162:, :, :] = seq_real[:, 162:, :, :] * params.stds_land[:,params.land_channels] + params.means_land[:,params.land_channels]
    ...
    seq_pred[:, 69:162, :, :] = seq_pred[:, 69:162, :, :] * params.stds_ocean[:,params.ocean_channels] + params.means_ocean[:,params.ocean_channels]
    seq_pred[:, 162:, :, :] = seq_pred[:, 162:, :, :] * params.stds_land[:,params.land_channels] + params.means_land[:,params.land_channels]
...
if __name__ == '__main__':
    ...
    params['best_checkpoint_path4'] = os.path.join(expDir, 'land/training_checkpoints/ckpt.tar')
    ...
    valid_data_full_atmos, valid_data_full_ocean, valid_data_full_sst, valid_data_full_land, model, model2, model4, model3, climate_mean = setup(params)
    ...
    for i, ic_ in enumerate(ics):
        ...
        seq_real, seq_pred = autoregressive_inference(params, ic_, valid_data_full_atmos, valid_data_full_ocean, valid_data_full_sst, valid_data_full_land, model, model2, model4, model3, climate_mean)
