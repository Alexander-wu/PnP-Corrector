...
class Trainer():
    def count_parameters(self):
        return sum(p.numel() for p in self.model3.parameters() if p.requires_grad) 
    ...
    def __init__(self, params, world_rank):
        ...
        with h5py.File(self.params.land_mask_path, 'r') as _f:
            self.mask_data = torch.as_tensor(_f['fields'])[0, :].to(self.device, dtype=torch.bool)
        ...
        self.train_data_loader, self.train_dataset, self.train_sampler = get_data_loader(
                ...
                params.train_data_path_ocean,
                params.train_data_path_land,
                dist.is_initialized(),
                train=True)
        self.valid_data_loader, self.valid_dataset, self.valid_sampler = get_data_loader(
                params, 
                params.valid_data_path,
                params.valid_data_path_ocean,
                params.valid_data_path_land,
                dist.is_initialized(), 
                train=True)
        ...
        self.model4 = model4(params).to(self.device)
        self.model3 = model3(params).to(self.device)


        if params.optimizer_type == 'FusedAdam':
            self.optimizer = torch.optim.Adam(self.model3.parameters(), lr = params.lr)
        else:
            self.optimizer = torch.optim.Adam(self.model3.parameters(), lr = params.lr)
        ...
        self.switch_off_grad(self.model2)

        if dist.is_initialized():
            self.model4 = DistributedDataParallel(
                    self.model4,
                    device_ids=[params.local_rank],
                    output_device=[params.local_rank],
                    find_unused_parameters=False
        ...
        self.switch_off_grad(self.model4)
        
        if dist.is_initialized():
            self.model3 = DistributedDataParallel(
                    self.model3,
                    device_ids=[params.local_rank],
                    output_device=[params.local_rank],
                    find_unused_parameters=False
        ...
        self.restore_checkpoint_model1(params.pretrained_ckpt_path_model1)
        print('model1 is ok (atmos)')
        self.restore_checkpoint_model2(params.pretrained_ckpt_path_model2)
        print('model2 is ok (ocean)')
        self.restore_checkpoint_model4(params.pretrained_ckpt_path_model4)
        print('model4 is ok (land)')
        ...
        for epoch in range(self.startEpoch, self.params.max_epochs):
            ...
            if self.world_rank == 0:
                if self.params.save_checkpoint:
                    ...
                    self.save_checkpoint(self.params.checkpoint_path, self.model3)
                    if valid_logs['valid_loss'] <= best_valid_loss:
                        logging.info('Val loss improved from {} to {}'.format(best_valid_loss, valid_logs['valid_loss']))
                        self.save_checkpoint(self.params.best_checkpoint_path, self.model3)
    ...
    def train_one_epoch(self):
        ...
        for i, data in enumerate(self.train_data_loader, 0):
            ...
            (inp_atmos, ocean, land, tar) = data
            # ic(inp.shape, tar.shape)
            ...
            with amp.autocast(self.params.enable_amp):
                ...
                for step_idx in range(num_steps):
                    if step_idx == 0:
                        inp_step_1_atmos = inp_atmos.to(self.device, dtype = torch.float32)
                        with torch.no_grad():
                            gen_model1 = self.model(inp_step_1_atmos)
                            atmos_forcing0 = inp_step_1_atmos[:, [65, 66, 67, 68], :, :]
                            atmos_forcing1 = gen_model1[:, [65, 66, 67, 68], :, :]

                            atmos_forcing_for_land = inp_step_1_atmos[:, [65, 66, 67], :, :]
                            
                            inp_model2 = torch.cat((ocean.cuda(), atmos_forcing0, atmos_forcing1), axis=1)
                            gen_model2 = self.model2(inp_model2)
                            gen_model2 = self.land_mask_func_single(gen_model2)

                            inp_model4 = torch.cat((land.cuda(), atmos_forcing_for_land), axis=1)
                            gen_model4 = self.model4(inp_model4)

                        inp_model3 = torch.cat((gen_model1, gen_model2, gen_model4), axis=1)
                        # gen_cur = checkpoint.checkpoint(self.model3, inp_model3, use_reentrant=False)
                        gen_cur = self.model3(inp_model3)
                        
                        
                    if params.multi_steps_finetune == 1:
                        tar_step = tar[:, :].to(self.device, dtype=torch.float)
                    else:
                        tar_step = tar[:, step_idx, :].to(self.device, dtype=torch.float)
                        
                    gen_cur[:, 69:162, :, :]= self.land_mask_func_single(gen_cur[:, 69:162, :, :])
                    tar_step[:, 69:162, :, :]= self.land_mask_func_single(tar_step[:, 69:162, :, :])
    ...
    def validate_one_epoch(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.valid_data_loader, 0):
                ...
                inp_atmos, ocean, land, tar = map(lambda x: x.to(self.device, dtype=torch.float), data)
                # gen = self.model(inp)
                num_steps = params.multi_steps_finetune
                for step_idx in range(num_steps):
                    if step_idx == 0:
    ...
    def restore_checkpoint_model1(self, checkpoint_path):
        """ We intentionally require a checkpoint_dir to be passed
            in order to allow Ray Tune to use this function """
    ...
    def restore_checkpoint_model2(self, checkpoint_path):
        """ We intentionally require a checkpoint_dir to be passed
            in order to allow Ray Tune to use this function """
        checkpoint = torch.load(checkpoint_path, map_location='cuda:{}'.format(self.params.local_rank))
        try:
            self.model2.load_state_dict(checkpoint['model_state'])
        except:
            new_state_dict = OrderedDict()
            for key, val in checkpoint['model_state'].items():
                name = key[7:]
                new_state_dict[name] = val
            self.model2.load_state_dict(new_state_dict)
        self.iters = checkpoint['iters']
        self.startEpoch = checkpoint['epoch']
        # if self.params.resuming:  
        # # restore checkpoint is used for finetuning as well as resuming. 
    ...
    def restore_checkpoint_model4(self, checkpoint_path):
        """ We intentionally require a checkpoint_dir to be passed
            in order to allow Ray Tune to use this function """
        checkpoint = torch.load(checkpoint_path, map_location='cuda:{}'.format(self.params.local_rank))
        try:
            self.model4.load_state_dict(checkpoint['model_state'])
        except:
            new_state_dict = OrderedDict()
            for key, val in checkpoint['model_state'].items():
                name = key[7:]
                new_state_dict[name] = val
            self.model4.load_state_dict(new_state_dict)
        self.iters = checkpoint['iters']
        self.startEpoch = checkpoint['epoch']
        # if self.params.resuming:  
        # # restore checkpoint is used for finetuning as well as resuming. 
...
if __name__ == '__main__':
    ...
    params['pretrained_ckpt_path_model1'] = os.path.join(pretrained_expDir, f'atmos/training_checkpoints/ckpt.tar')
    params['pretrained_ckpt_path_model2'] = os.path.join(pretrained_expDir, f'ocean/training_checkpoints/best_ckpt.tar')
    params['pretrained_ckpt_path_model4'] = os.path.join(pretrained_expDir, f'land/training_checkpoints/ckpt.tar')

    expDir = os.path.join(pretrained_expDir, 'coupler')
    if world_rank == 0:
