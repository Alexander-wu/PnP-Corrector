            ...
            with amp.autocast(self.params.enable_amp):
                ...
                for step_idx in range(num_steps):
                    if step_idx == 0:
                        ...
                        inp_model3 = torch.cat((gen_model1, gen_model2), axis=1)
                    ...
                    gen_cur[:, 69:, :, :]= self.land_mask_func_single(gen_cur[:, 69:, :, :])
                    tar_step[:, 69:, :, :]= self.land_mask_func_single(tar_step[:, 69:, :, :])
    ...
    def validate_one_epoch(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.valid_data_loader, 0):
                ...
                inp_atmos, ocean, tar = map(lambda x: x.to(self.device, dtype=torch.float), data)
                # gen = self.model(inp)
                num_steps = params.multi_steps_finetune
                for step_idx in range(num_steps):
                    if step_idx == 0:
