...
def reshape_fields_forcing(img, inp_or_tar, params, train, normalize=True, orog=None, add_noise=False):
    # Takes in np array of size (n_history+1, c, h, w) 
    # returns torch tensor of size ((n_channels*(n_history+1), crop_size_x, crop_size_y)
    ...
    if inp_or_tar == 'inp':
        ...
        channels = params.forcing_channels
    else:
        channels = params.out_channels
    ...
    if normalize and params.normalization == 'zscore':
        means = np.load(params.global_means_path_forcing)[:, channels]
        stds = np.load(params.global_stds_path_forcing)[:, channels]
        img -=means
        img /=stds
