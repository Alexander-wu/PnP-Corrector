...
import math
# import cv2
from my_utils.norm import reshape_fields, reshape_fields_forcing



def get_data_loader(params, files_pattern, files_pattern_forcing, distributed, train):
    dataset = GetDataset(params, files_pattern, files_pattern_forcing, train)
...
class GetDataset(Dataset):
    def __init__(self, params, location, location_forcing, train):
        self.params = params
        self.location = location
        self.location_forcing = location_forcing
        self.train = train
        ...
        self.forcing_channels = np.array(params.forcing_channels)
    ...
    def _get_files_stats(self):
        ...
        self.files_paths_forcing = glob.glob(self.location_forcing + "/*.h5")
        self.files_paths.sort()
        self.files_paths_forcing.sort()
        self.n_years = len(self.files_paths)
        logging.info('files_paths: %s' % self.files_paths)
        logging.info('files_paths_forcing: %s' % self.files_paths_forcing)
        ...
        self.files_forcing = [None for _ in range(self.n_years)]
    ...
    def _open_file(self, year_idx):
        ...
        _file_forcing = h5py.File(self.files_paths_forcing[year_idx], 'r')
        self.files[year_idx] = _file['fields']
        self.files_forcing[year_idx] = _file_forcing['fields'] 
    ...
    def __getitem__(self, global_idx):
        ...
        if self.params.multi_steps_finetune == 1:
            ...
            forcing = reshape_fields_forcing( 
                    self.files_forcing[year_idx][(local_idx-self.dt*self.n_history):(local_idx+1):self.dt, self.forcing_channels, :180, :360], 
                    'force', 
                    self.params, 
                    self.train, 
                    self.normalize, 
            ...
            inp = reshape_fields( 
                    np.nan_to_num(self.files[year_idx][(local_idx-self.dt*self.n_history):(local_idx+1):self.dt, self.in_channels, :180, :360], nan=0), 
                    'inp', 
                    self.params, 
                    self.train, 
                    self.normalize, 
            ...
            tar = reshape_fields(
                    np.nan_to_num(self.files[year_idx][local_idx+step, self.out_channels, :180, :360], nan=0), 
                    'tar', 
                    self.params, 
                    self.train, 
                    self.normalize, 
        ...
        return np.concatenate((inp, forcing), axis=0), tar 
